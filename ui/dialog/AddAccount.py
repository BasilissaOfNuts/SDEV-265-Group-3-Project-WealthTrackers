...
class AddAccount(QDialog):
    def __init__(self, parent=None):
        ...
        # Keeping the 450px width as requested
        ...
        # Populate the checking accounts for the dropdown
        self.populate_checking_accounts()
        # Initialize height for the default "Checking" selection
        self.switch_dynamic_fields(0)
    ...
    def setup_ui(self):
        ...
        # 0: Checking
        chk_w = QWidget()
        chk_l = QVBoxLayout(chk_w)
        chk_l.setContentsMargins(0, 0, 0, 0)
        self.routing_input = QLineEdit(placeholderText="Routing Number")
        chk_l.addWidget(self.routing_input)
        self.dynamic_stack.addWidget(chk_w)

        # 1: Savings
        sav_w = QWidget()
        sav_l = QVBoxLayout(sav_w)
        sav_l.setContentsMargins(0, 0, 0, 0)
        self.interest_input = QLineEdit(placeholderText="Interest Rate (e.g., 0.04)")
        sav_l.addWidget(self.interest_input)
        self.dynamic_stack.addWidget(sav_w)

        # 2: Credit Card
        cc_w = QWidget()
        cc_l = QVBoxLayout(cc_w)
        cc_l.setContentsMargins(0, 0, 0, 0)
        self.cc_cvv = QLineEdit(placeholderText="CVV")
        self.cc_limit = QLineEdit(placeholderText="Credit Limit")
        self.cc_apr = QLineEdit(placeholderText="APR (e.g., 0.24)")
        cc_l.addWidget(self.cc_cvv)
        cc_l.addWidget(self.cc_limit)
        cc_l.addWidget(self.cc_apr)
        self.dynamic_stack.addWidget(cc_w)

        # 3: Debit Card
        dc_w = QWidget()
        dc_l = QVBoxLayout(dc_w)
        dc_l.setContentsMargins(0, 0, 0, 0)
        self.dc_cvv = QLineEdit(placeholderText="CVV")
        self.dc_checking_selector = QComboBox()
        self.dc_checking_selector.setStyleSheet("color: #333333; background-color: white;")
        dc_l.addWidget(self.dc_cvv)
        dc_l.addWidget(QLabel("Link to Checking Account:"))
        dc_l.addWidget(self.dc_checking_selector)

        self.dynamic_stack.addWidget(dc_w)
    ...
    def populate_checking_accounts(self):
        """Fetches checking accounts and stores UUID in ComboBox data."""
        ...
        try:
            ...
            cursor = conn.cursor()
            # Fetch account name, encrypted number, and ID for matching
            cursor.execute("SELECT account_name, enc_acc_num, account_id FROM accounts WHERE account_type = 'CHECKING'")

            self.dc_checking_selector.clear()

            for account_name, enc_num, acc_id in cursor.fetchall():
                dec_num = AuthenticationService.decrypt(enc_num, session_key)
                display_name = f"{account_name} (****{dec_num[-4:]})"
                # Store the UUID directly as item data
                self.dc_checking_selector.addItem(display_name, acc_id)

            conn.close()
        except Exception as e:
            print(f"Error loading checking accounts: {e}")

    def switch_dynamic_fields(self, index):
        for i in range(self.dynamic_stack.count()):
            widget = self.dynamic_stack.widget(i)
            if i == index:
        ...
        # Hide the balance input ONLY if Debit Card is selected
        if self.type_dropdown.itemText(index) == "Debit Card":
            self.balance_input.hide()
        else:
            self.balance_input.show()
    ...
    def build_account(self):
        ...
        # Hardcode balance to 0 for debit cards only
        if acc_type == "Debit Card":
        ...
        try:
            if acc_type == "Checking":
                ...
                # Retrieve the UUID directly from the dropdown data
                linked_checking_uuid = self.dc_checking_selector.currentData()

                if not linked_checking_uuid:
                    QMessageBox.warning(self, "Error", "You must select a Checking account to link.")
                    return

                self.new_account = DebitCardAccount(acc_id, name, balance, enc_acc_num, enc_cvv, linked_checking_uuid)
